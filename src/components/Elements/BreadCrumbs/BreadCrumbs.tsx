...
export default function Breadcrumb({ labels, page, onLinkClick }: BreadcrumbType) {
  return (
    <nav
    ...
    >
      <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
        {Object.keys(labels).map((key: string, index: number) => (
          <li className="inline-flex items-center" key={index}>
            {page !== labels[key] ? <a href=""
              onClick={() => onLinkClick(labels[key])}
